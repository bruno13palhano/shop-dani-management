...
internal class CategoryRepository @Inject constructor(
    ...
    override suspend fun deleteById(id: Long) {
        versionLight.update(DataVersion(1L, "CATEGORY", OffsetDateTime.now()))
    ...
    override suspend fun syncWith(synchronizer: Synchronizer): Boolean =
        synchronizer.syncData(
            dataVersion = getDataVersion(versionLight, 1L),
    ...
    override suspend fun update(model: Category) {
        versionLight.update(DataVersion(1L, "CATEGORY", model.timestamp))
        return categoryData.update(model = model)
    }

    override suspend fun insert(model: Category): Long {
        versionLight.insert(DataVersion(1L, "CATEGORY", model.timestamp))
