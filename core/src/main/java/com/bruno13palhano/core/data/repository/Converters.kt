...
fun catalogToCatalogNet(model: Catalog) = CatalogNet (
    id = model.id,
...
fun catalogNetToCatalog(model: CatalogNet) = Catalog(
    id = model.id,
    productId = model.productId,
    name = "",
    photo = byteArrayOf(),
...
fun categoryNetToCategory(categoryNet: CategoryNet) = Category(
    id = categoryNet.id,
    category = categoryNet.category,
    timestamp = categoryNet.timestamp
)

fun categoryToCategoryNet(category: Category) = CategoryNet(
    id = category.id,
    category = category.category,
    timestamp = category.timestamp
)

fun customerNetToCustomer(customerNet: CustomerNet) = Customer(
    id = customerNet.id,
...
fun customerToCustomerNet(customer: Customer) = CustomerNet(
    id = customer.id,
    name = customer.name,
...
fun productToProductNet(product: Product) = ProductNet(
    id = product.id,
    name = product.name,
    code = product.code,
...
fun productNetToProduct(productNet: ProductNet) = Product(
    id = productNet.id,
...
fun versionToVersionNet(version: DataVersion) = DataVersionNet(
    id = version.id,
    name = version.name,
    timestamp = version.timestamp
)

fun versionNetToVersion(versionNet: DataVersionNet) = DataVersion(
    id = versionNet.id,
    name = versionNet.name,
    timestamp = versionNet.timestamp
)

fun saleToSaleNet(sale: Sale) = SaleNet(
    id = sale.id,
...
fun saleNetToSale(saleNet: SaleNet) = Sale(
    id = saleNet.id,
...
fun stockItemToStockItemNet(stockItem: StockItem) = StockItemNet(
    id = stockItem.id,
...
fun stockItemNetToStockItem(stockItemNet: StockItemNet) = StockItem(
    id = stockItemNet.id,
...
fun userNetToUser(userNet: UserNet) = User(
    id = userNet.id,
...
fun userToUserNet(user: User) = UserNet(
    id = user.id,
